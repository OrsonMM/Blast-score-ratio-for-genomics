...
def extract_bsr_reference(data_base,cores):
	...
	file = subprocess.check_output("blastn -db %s -query %s -outfmt 6 -num_alignments 5 -perc_identity 100 -qcov_hsp_perc 100 -num_threads %d"%(data_base,data_base,int(cores)),shell=True)
...
def main():
	...
	funcion_makeblast(data_base)           # These function do makeblastdb with multifasta input, return 

	nucle_files = do_prodigal(genomes)     # These function predicted CDS using prodigal and return the new dir with cds. 

	do_blastn(nucle_files,data_base,cores) # These functoin make a blastn cds predictef with prodigal vs your genes in multifasta.
